...
function setAttr(element, params){
	...
	for(var attr in params){
		...
		}else if (attr == 'left' || attr == 'top' || attr == 'width' || attr == 'height'){
			setStyleInt(element, attr, parseInt(params[attr]));
		}else{
			element.style[attr] = params[attr];
		}
	}
...
function relocatePolar(element, polarParams){
	...
	if(checkBoundary(element, x, y)){
		element.outOfBoundary = true;		//给player做判断用，这句写的也不地道
		return;				 
	}
	setStyleInt(element, 'left', x);
	setStyleInt(element, 'top', y);
}
function checkBoundary(element, x, y){		//这个地方要注意，没有出现问题是因为目前plane方式没有用这个做判断
	var width = element.offsetWidth,
		height = element.offsetHeight,
		parent = element.parentNode;
	var parentWidth = 400,					//暂时写死，还是chrome强壮一些
		parentHeight = 500;
	if(x < -width || x > parentWidth + width){
		parent.removeChild(element);
		return true;
	}
	if(y < -40 || y > parentHeight + 40){
		parent.removeChild(element);
		return true;
	}
	return false;
